/*
  # Secret Rotation Tracking System
  ...
  1. New Tables
    - `secret_rotation_tracking`
      ...
      - `next_rotation_due` (timestamp, calculated)
      - `is_critical` (boolean)
      - `rotation_notes` (text)
  ...
  2. Security
    - Enable RLS on `secret_rotation_tracking` table
    - Add policies for admin-only access to view, update, and insert records

  3. Functions
    - `update_secret_rotation_due()` - Automatically calculates next rotation date
    - `mark_secret_rotated()` - Records when a secret is rotated
    - `get_secrets_needing_rotation()` - Returns overdue secrets

  4. Initial Data
    - Pre-populates tracking for critical secrets: Stripe, PayPal, Resend, Instagram, etc.
*/
...
DROP POLICY IF EXISTS "Admins can view secret rotation tracking" ON public.secret_rotation_tracking;
...
DROP POLICY IF EXISTS "Admins can update secret rotation tracking" ON public.secret_rotation_tracking;
...
DROP POLICY IF EXISTS "Admins can insert secret rotation tracking" ON public.secret_rotation_tracking;
...
DROP TRIGGER IF EXISTS calculate_next_rotation_due ON public.secret_rotation_tracking;
...
BEGIN
  ...
  -- Log security event (will fail silently if function doesn't exist)
  BEGIN
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Ignore if log_security_event doesn't exist
    NULL;
  END;
