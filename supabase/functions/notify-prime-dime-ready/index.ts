...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch athletes who need notification (without profile join to avoid schema cache issues)
    ...
    if (athletesError) {
      logStep("ERROR fetching athletes", { error: athletesError.message, details: athletesError });
      throw athletesError;
    }
    ...
    for (const athlete of athletes) {
      try {
        ...
        if (profileError) {
          logStep("Error fetching profile", { athleteId: athlete.id, error: profileError.message });
          errorCount++;
          continue;
        }
    ...
    const errorDetails = error instanceof Error 
      ? JSON.stringify({ message: error.message, name: error.name, cause: error.cause }, null, 2)
      : JSON.stringify(error, null, 2);
    
    logStep("ERROR", { message: errorMessage, stack: errorStack, details: errorDetails });
    
    // Send error notification to tech support with properly serialized error
    try {
      const resend = new Resend(Deno.env.get("RESEND_API_KEY"));
      await resend.emails.send({
        ...
        html: `
          ...
          <pre style="background: #f4f4f4; padding: 10px; border-radius: 4px; overflow-x: auto;">${errorDetails}</pre>
          <p><strong>Stack Trace:</strong></p>
          <pre style="background: #f4f4f4; padding: 10px; border-radius: 4px; overflow-x: auto;">${errorStack}</pre>
        `,
      });
    } catch (notifyError) {
      console.error("Failed to send error notification:", notifyError instanceof Error ? notifyError.message : String(notifyError));
    }
